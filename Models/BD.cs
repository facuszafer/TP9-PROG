...
namespace TP9
{
    ...
    {
        private static string _connectionString = @"Server=A-PHZ2-CIDI-016  ;DataBase=TP9;Trusted_Connection=True;";
        ...
        public static ropa VerInfoPrecio(int IdRop)
        {
            ropa precioActual = null;
            using (SqlConnection DB = new SqlConnection(_connectionString))
            {
                string sql = "SELECT * FROM ropa WHERE IdRopa = @pIdRopa";
                precioActual = DB.QueryFirstOrDefault<ropa>(sql, new { pIdRopa = IdRop });
            }
            return precioActual;
        }
